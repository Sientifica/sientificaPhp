    ...
    protected function __construct($info) {
        ...
        try {
            ...
            if ($info->protocol == 'mysql') {
                $this->connection->query("SET NAMES '".$config->configApp->db['charset']."'");
                $this->connection->query("SET CHARACTER SET ".$config->configApp->db['charset']."");
                $this->connection->query("SET character_set_connection=".$config->configApp->db['charset']."");
                $this->connection->query("SET character_set_client=".$config->configApp->db['charset']."");
            }
        } catch (PDOException $e) {
            throw new DatabaseException($e);
        }
